...
static esp_err_t environment_handler(httpd_req_t *req) {
  ...
  if (g_dhtData.valid) {
    float tempC = g_dhtData.temperature;
    float tempF = (tempC * 9.0 / 5.0) + 32.0;
    float hum = g_dhtData.humidity;
    snprintf(json_response, sizeof(json_response),
      "{\"temperature_c\":%.1f,\"temperature_f\":%.1f,\"humidity_pct\":%.1f,\"sensor_online\":true,\"sensor_type\":\"DHT11\",\"gpio\":13,\"device_id\":\"LIFELINE-HOME-NODE-02\"}",
      tempC, tempF, hum);
  } else {
    snprintf(json_response, sizeof(json_response),
      "{\"temperature_c\":null,\"temperature_f\":null,\"humidity_pct\":null,\"sensor_online\":false,\"sensor_type\":\"DHT11\",\"gpio\":13,\"device_id\":\"LIFELINE-HOME-NODE-02\"}");
  }
...
#endif
  if (g_dhtData.valid) {
    p += snprintf(p, end - p, ",\"environment\":{\"temperature_c\":%.1f,\"humidity_pct\":%.1f,\"sensor_online\":true}",
                  g_dhtData.temperature, g_dhtData.humidity);
  } else {
    p += snprintf(p, end - p, ",\"environment\":{\"temperature_c\":null,\"humidity_pct\":null,\"sensor_online\":false}");
  }
  *p++ = '}';
  *p++ = 0;
  httpd_resp_set_type(req, "application/json");
